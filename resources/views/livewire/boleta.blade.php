<div class="bg-white overflow-hidden shadow-md rounded-lg max-w-7xl mx-auto sm:px-6 lg:p-8 ">
    <div class="rounded border border-gray-300 shadow-md">
        @for ($i = 0; $i < $longitud; $i++)
            ...
            <p class="text-bold text-center w-full bg-slate-400 px-2" >{{$calificaciones[$i]->clase->tipo_semestre == ( $i>0 ? $calificaciones[$i-1]->clase->tipo_semestre  : 0 ) ? null : $calificaciones[$i]->clase->tipo_semestre }}</p>
            @if ($i==0)
